...
const BrandPage = () => {
  ...
  const {
    ...
    queryKey: [
      `/api/v1/brand/dashboard?page=${page}&limit=${limit}&searchTerm=${searchTerm}`,
    ],
    queryFn: async () => {
      try {
        const res = await fetch(
          `${BASE_URL}/brand/dashboard?page=${page}&limit=${limit}&searchTerm=${searchTerm}`,
          {
            credentials: "include",
          }
        );
  ...
  return (
    <div className="py-6 px-1">
      <div className="flex justify-between mt-6">
        <div>
          <h1 className="text-2xl text-primaryColor uppercase">Brand</h1>
        </div>

        {user?.role_id?.brand_post === true && (
          <div>
            <button
              type="button"
              ...
              onClick={() => setBrandCreateModal(true)}
            >
              Create Brand
            </button>
          </div>
      ...
      {/* Sub Brand Data Show and update and delete operation file */}
